...
export class Endpoints {
  ...
  public composeEmail = async (uncheckedReq: unknown): Promise<EndpointRes> => {
    ...
    if (req.replyToMsgId) {
      ...
      mimeHeaders.references = [req.inReplyTo, req.replyToMsgId].filter(value => !!value).join(' ');
    }

    if (req.format === 'plain') {
      const atts = (req.atts || []).map(
        ({ name, type, base64 }) => new Att({ name, type, data: Buf.fromBase64Str(base64) }),
      );
      // eslint-disable-next-line @typescript-eslint/naming-convention
  ...
  public parseDecryptMsg = async (uncheckedReq: unknown, data: Buffers): Promise<EndpointRes> => {
    ...
    for (const block of sequentialProcessedBlocks) {
      ...
      if (block.content instanceof Buf) {
    ...
    const blocksData = Buf.fromUtfStr(
      blocks
        .map(b => {
