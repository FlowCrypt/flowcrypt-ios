...
final class Core {
    ...
    private static dynamic var started = false;
    private static dynamic var ready = false;

    public static func version() throws -> CoreRes.Version {
        let r = try self.call("version", jsonDict: nil, data: nil)
    ...
    public static func parseKeys(armoredOrBinary: Data) throws -> CoreRes.ParseKeys {
        let r = try self.call("parseKeys", jsonDict: [String: String](), data: armoredOrBinary)
    ...
    public static func parseDecryptMsg(encrypted: Data, keys: [PrvKeyInfo], msgPwd: String?, isEmail: Bool) throws -> CoreRes.ParseDecryptMsg {
        let parsed = try self.call("parseDecryptMsg", jsonDict: [ "keys": try keys.map { try $0.toDict() },  "isEmail": isEmail, "msgPwd": msgPwd ], data: encrypted)
    ...
    public static func generateKey(passphrase: String, variant: KeyVariant, userIds: [UserId]) throws -> CoreRes.GenerateKey {
        ...
        let r = try self.call("generateKey", jsonDict: request, data: nil)
...
//    }
    ...
    private static func call(_ endpoint: String, jsonDict: [String: Any?]?, data: Data?) throws -> RawRes {
        return try self.call(endpoint, jsonData: try JSONSerialization.data(withJSONObject: jsonDict ?? [String: String]()), data: data ?? Data())
    }

    private static func call(_ endpoint: String, jsonEncodable: Encodable, data: Data) throws -> RawRes {
        return try self.call(endpoint, jsonData: try jsonEncodable.toJsonData(), data: data)
    }

    private static func call(_ endpoint: String, jsonData: Data, data: Data) throws -> RawRes {
        try self.blockUntilReadyOrThrow()
        cb_last_value = nil;
        self.jsEndpointListener!.call(withArguments: [endpoint, String(data: jsonData, encoding: .utf8)!, data.base64EncodedString(), cb_catcher!])
